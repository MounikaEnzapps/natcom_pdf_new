...
class AccountMove(models.Model):
    ...
    curr_code = fields.Char(string='CURR_CODE')
    word_without_vat_english = fields.Char(string="Word without vat english")
    word_with_vat_english = fields.Char(string="Word with vat english")


    def update_all_invoice(self):
            ...
            if responce:
                ...
                for line in line_data:
                    # if invoice_length <= link_no:
                        ...
                        if old_invoice:
                            if not old_invoice.word_without_vat_english:
                               print(old_invoice,'111')
                               old_invoice.partner_id.cust_code = line['CUST_CODE']
                               old_invoice.curr_code = line['CURR_CODE']
                               old_invoice.word_without_vat_english = line['Word without vat english']
                               old_invoice.word_with_vat_english = line['Word with vat english']
...
class ResPartner(models.Model):
    ...
    cust_code = fields.Char(string="CUST_CODE")
